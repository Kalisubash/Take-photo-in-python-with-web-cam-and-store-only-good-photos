...
output_dir = r"C://Users//ransu//Documents//Python//outputphoto//sanchit1"
...
while photo_count < 20:
    ...
    if len(faces) > 0:
        ...
        # Check if the saved image is feasible for face recognition
        image = face_recognition.load_image_file(photo_filename)
        face_encodings = face_recognition.face_encodings(image)
        
        if len(face_encodings) == 0:
            # If no face encodings are found, delete the file
            os.remove(photo_filename)
            print(f"Deleted: {photo_filename} (No recognizable face found)")
